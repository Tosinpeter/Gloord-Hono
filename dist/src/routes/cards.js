...
import { savedCards } from "../db/schema.js";
...
export function validateMastercard(number) {
...
cardsRoute.post("/", async (c) => {
    ...
    if (num.length !== 16) {
        return c.json({ success: false, error: "invalid_number", message: "Card number must be 16 digits" }, 400);
    }
    const validation = validateMastercard(body.number);
    if (!validation.valid) {
        return c.json({ success: false, error: "invalid_card", message: validation.error }, 400);
    }
    ...
    if (!Number.isInteger(exp_month) || exp_month < 1 || exp_month > 12) {
        return c.json({ success: false, error: "invalid_expiry", message: "Invalid expiry month" }, 400);
    }
    const now = new Date();
    const currentYear = now.getFullYear() % 100;
    const currentMonth = now.getMonth() + 1;
    if (exp_year < currentYear || (exp_year === currentYear && exp_month < currentMonth)) {
        return c.json({ success: false, error: "expired", message: "Card has expired" }, 400);
    }
...
cardsRoute.post("/charge", async (c) => {
    ...
    if (!Number.isInteger(amount_cents) || amount_cents <= 0) {
        return c.json({ success: false, error: "invalid_amount", message: "Invalid amount" }, 400);
    }
    let last4;
    if (body.card_id) {
        ...
        return c.json({ success: false, error: "missing_card", message: "Provide card_id or number" }, 400);
    }
